-- Also offer "Historia Clínica" at the client level (not just per-appointment).
-- A template row is scoped to a single `type`, so this is a second row sharing
-- the same body fields, with a header adjusted to drop appointment/treatment
-- prefills that don't apply outside an appointment context (mirrors how
-- physio_initial_assessment's client-level header is built).

DO $$
DECLARE
  c RECORD;
  hc_client_schema JSONB;
BEGIN
  hc_client_schema := jsonb_build_object(
    'layout', 'simple',
    'sections', jsonb_build_array(
      ...
      jsonb_build_object('id', 'reference', 'label', 'Referencia', 'type', 'textarea'),
      jsonb_build_object('id', 'medical_diagnosis', 'label', 'Diagnóstico Médico', 'type', 'textarea'),
      jsonb_build_object('id', 'observations', 'label', 'Observaciones', 'type', 'textarea'),
      jsonb_build_object('id', 'inflammation', 'label', 'Inflamación', 'type', 'scale', 'min', 1, 'max', 4),
      jsonb_build_object('id', 'clinical_history', 'label', 'Cuadro Clínico / Antecedentes', 'type', 'textarea'),
      jsonb_build_object('id', 'pain_level', 'label', 'Nivel de Dolor', 'type', 'scale', 'min', 0, 'max', 10)
    )
  );
  ...
  FOR c IN SELECT id FROM public.clinics LOOP
    ...
    SELECT c.id, 'clinical_history_client', 'Historia Clínica',
      'Historia clínica del paciente: diagnóstico, observaciones, antecedentes y escalas de inflamación y dolor.',
      'client', 'assessment', 'es', 1, true, hc_client_schema
    WHERE NOT EXISTS (
      SELECT 1 FROM public.document_templates dt
      WHERE dt.clinic_id = c.id AND dt.slug = 'clinical_history_client' AND dt.version = 1
    );
  END LOOP;
END $$;

-- Register it in the seed-on-new-clinic function too (append a 9th block, keep 1-8 unchanged).
...
BEGIN
    ...
    'sections', jsonb_build_array(
      jsonb_build_object(
        ...
        'fields', jsonb_build_array(
          jsonb_build_object('id', 'client_name', 'label', 'Paciente', 'type', 'text', 'prefillFrom', 'client.full_name', 'readonly', true),
          jsonb_build_object('id', 'therapist_name', 'label', 'Fisioterapeuta', 'type', 'text', 'prefillFrom', 'therapist.full_name', 'readonly', true),
          jsonb_build_object('id', 'session_date', 'label', 'Fecha de sesión', 'type', 'text', 'prefillFrom', 'appointment.date', 'readonly', true),
          jsonb_build_object('id', 'treatment_name', 'label', 'Tratamiento', 'type', 'text', 'prefillFrom', 'treatment.name', 'readonly', true)
        )
      ),
      jsonb_build_object('id', 'reference', 'label', 'Referencia', 'type', 'textarea'),
      jsonb_build_object('id', 'medical_diagnosis', 'label', 'Diagnóstico Médico', 'type', 'textarea'),
  ...
  INSERT INTO public.document_templates (clinic_id, slug, name, description, type, category, language, version, is_active, schema)
  VALUES (NEW.id, 'clinical_history', 'Historia Clínica',
    'Historia clínica de sesión: diagnóstico, observaciones, antecedentes y escalas de inflamación y dolor.',
    'appointment', 'session_note', 'es', 1, true, hc_schema)
  ON CONFLICT DO NOTHING;

  INSERT INTO public.document_templates (clinic_id, slug, name, description, type, category, language, version, is_active, schema)
  VALUES (NEW.id, 'clinical_history_client', 'Historia Clínica',
    'Historia clínica del paciente: diagnóstico, observaciones, antecedentes y escalas de inflamación y dolor.',
    'client', 'assessment', 'es', 1, true, hc_client_schema)
  ON CONFLICT DO NOTHING;
