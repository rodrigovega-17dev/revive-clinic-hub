-- Adds a new "Historia Clínica" appointment document template with a custom
-- 1-4 inflammation scale and a 0-10 pain scale (new 'scale' field type,
-- rendered by the frontend; min/max are just extra schema keys on the field).
...
BEGIN
  ...
  FOR c IN SELECT id FROM public.clinics LOOP
    ...
    SELECT c.id, 'clinical_history', 'Historia Clínica',
    ...
    WHERE NOT EXISTS (
      ...
      WHERE dt.clinic_id = c.id AND dt.slug = 'clinical_history' AND dt.version = 1
    );
  END LOOP;
END $$;

-- Register it in the seed-on-new-clinic function too.
