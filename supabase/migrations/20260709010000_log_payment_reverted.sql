-- Extend the existing appointment activity-log trigger to also log when a payment is
-- reverted (payment_status goes from 'paid' back to something else). The trigger already
-- logs the opposite transition ('payment_recorded'); this adds the mirror event so
-- reverts leave an audit trail instead of silently disappearing from the log.
...
BEGIN
  ...
  -- Mirror of the block above: payment_status leaving 'paid' means a payment was reverted.
  IF (OLD.payment_status IS DISTINCT FROM NEW.payment_status) AND OLD.payment_status = 'paid' AND NEW.payment_status IS DISTINCT FROM 'paid' THEN
    v_amount := '$' || COALESCE(TO_CHAR(OLD.payment_amount, 'FM999,999,990.00'), '0');
    v_method_es := CASE OLD.payment_method::text
      WHEN 'cash'      THEN 'Efectivo'
      WHEN 'card'      THEN 'Tarjeta'
      WHEN 'transfer'  THEN 'Transferencia'
      WHEN 'insurance' THEN 'Seguro'
      WHEN 'balance'   THEN 'Balance'
      ELSE COALESCE(OLD.payment_method::text, '') END;
