...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_clinic_subscriptions_clinic_status 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_clinic_subscriptions_active_trial 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_clinic_subscriptions_plan_id 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_subscription_invoices_clinic_date 
ON subscription_invoices (clinic_id, created_at DESC);

-- Add index for subscription invoices by status for filtering
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_subscription_invoices_status 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_therapists_clinic_active 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_clients_clinic_active 
ON clients (clinic_id, is_active) 
WHERE is_active = true;

-- Add index for subscription usage tracking
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_subscription_usage_subscription_date 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_clinic_subscriptions_current_period 
ON clinic_subscriptions (clinic_id, current_period_end) 
WHERE status = 'active' AND current_period_end > NOW();

-- Add index for canceled subscriptions that haven't ended yet
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_clinic_subscriptions_canceled_pending 
ON clinic_subscriptions (clinic_id, current_period_end) 
WHERE cancel_at_period_end = true AND current_period_end > NOW();
