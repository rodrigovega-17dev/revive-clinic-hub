...
// Therapist payroll is based on the value of the session itself (its assigned price), never
// on how much was actually collected for it — a discount at checkout, a partial/installment
// payment, or even a stray $0 payment record must not silently change what the therapist is
// owed. totalCollected/totalIva are the real cash and tax figures (used for revenue/IVA
// reporting elsewhere) and are unaffected by this.
const summarizeAppointmentPayments = (appointment) => {
  ...
  if (paymentRows.length === 0) {
    return { totalCollected: preIvaRevenue, preIvaRevenue, totalIva: 0 };
  }
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        start_date: { type: 'string', description: 'ISO date.' },
        end_date: { type: 'string', description: 'ISO date.' },
        category: { type: 'string', description: 'e.g. supplies, office, maintenance, utilities, equipment, marketing, travel, food, taxes, payroll_contributions, professional_services, imss, payroll_percentage, general, Payroll.' },
