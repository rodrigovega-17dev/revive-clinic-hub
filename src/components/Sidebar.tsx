...
export const Sidebar = ({ mobileOpen = false, onMobileClose }: SidebarProps) => {
  ...
  return (
    <div
      className={cn(
        "flex flex-col h-screen bg-sidebar border-r border-sidebar-border shadow-lg transition-all duration-300 ease-in-out",
        // Mobile: fixed overlay, always w-64, slides in/out
        "fixed inset-y-0 left-0 z-50 w-64",
        mobileOpen ? "translate-x-0" : "-translate-x-full",
        // Desktop: static position, no translation, width based on collapsed
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 pt-[calc(1rem+env(safe-area-inset-top))] border-b border-sidebar-border bg-sidebar/50">
        ...
        <Button
          ...
          className="hover:bg-sidebar-accent text-sidebar-foreground/70 hover:text-sidebar-foreground hidden md:flex"
        >
      ...
      <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
        {navigation.map((item) => {
          ...
          return (
            ...
            >
              <item.icon className={cn("w-5 h-5 shrink-0", collapsed ? "md:mr-0 mr-3" : "mr-3", isActive ? "text-sidebar-primary-foreground" : "")} />
              <span className={cn(collapsed ? "md:hidden" : "")}>{t(item.name)}</span>
            </Button>
          );
        })}
      </nav>
        ...
        {!collapsed ? (
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3 flex-1 min-w-0">
              <div className="w-10 h-10 bg-gradient-to-br from-muted to-muted/80 rounded-full flex items-center justify-center shrink-0">
            ...
            <Button
              ...
              className="hover:bg-red-500/10 hover:text-red-500 text-sidebar-foreground/70 p-2 shrink-0"
          ...
          <Button
            ...
            size="sm"
            onClick={handleLogout}
            className="w-full hover:bg-red-500/10 hover:text-red-500 text-sidebar-foreground/70"
            title={t('navigation.logout')}
          >
            <LogOut className="w-4 h-4" />
          </Button>
